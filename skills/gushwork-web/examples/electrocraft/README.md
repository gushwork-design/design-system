# ElectroCraft — motor & pump vertical case study

Copied from `templates/case-study/` on 15 Sep 2026 and retargeted at motor and pump
manufacturers, with **ElectroCraft** (electrocraft.com) as the client.

## ⚠ This page carries SAMPLE outcome data — do not publish as-is

| Part | Status |
|---|---|
| Company facts — product lines, served markets, USA/Germany/China/Taiwan locations | **Real**, read off electrocraft.com on 15 Sep 2026 |
| Industry framing — how OEM engineers search by spec, the TL;DR problem/challenge/solution | **Real** vertical reasoning, written for this page |
| Hero title's `85+`, the three hero stats, both 90-day charts | **SAMPLE** — invented to show the layout |
| Pull quote | **SAMPLE** — nobody said this |
| Byline `[Name]` / `[Title], ElectroCraft` | **Deliberate placeholder**, left visible |

Three things were held back on purpose. They are no longer judgement calls — they are
**R24** in `DECISIONS.md`, ruled 15 Sep 2026, and this page is the worked instance the
ruling points at:

1. **The byline is bracketed, not filled.** A sample quote attributed to an invented named
   person is the one thing on this page that could not be walked back if it shipped.
2. **The byline photo is gone**, not swapped. The template shipped a real customer's
   photograph; borrowing a real face for a sample quote is worse than an empty slot.
3. **The hero logo overlay is deliberately unresolved.** ElectroCraft's real mark is
   fetchable, but a real company logo above SAMPLE numbers reads as a real endorsement.
   The `onerror` handler hides the box, exactly as the template intends. Add the logo only
   once the numbers and quote are real.

## Hero photography

`assets/case-study/sample/electrocraft-motor-stator.jpg` — motor stator windings and
commutator. Unsplash (free licence, no Unsplash+), photo `DNp9e_KuqIU` by Bruno Mira,
downloaded at 2400px and downscaled to 1100×733 with `sips -Z 1100` per the sourcing rule
below. Replace with real ElectroCraft plant photography when it exists.

## Still to do before this is a real page

- Real client name confirmation, three real outcome numbers, a real cleared quote + byline
- Re-render the OG image: `og-source.html` is updated, but `electrocraft-og.png` has not
  been generated yet, so the `og:image` / `twitter:image` URLs currently 404
- Add `electrocraft-logo.png` if and when the case is real
